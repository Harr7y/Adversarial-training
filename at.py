...
parser.add_argument('--fre_loss', default=False, type=bool, help='logits frequency pair loss')
parser.add_argument('--fre_rate', default=0.1, type=float, help='rate of frequency loss')

# weight average
parser.add_argument('--swa', action='store_true', help='swa usage flag (default: off)')
parser.add_argument('--swa_start', type=float, default=75, metavar='N', help='SWA start epoch number (default: 75)')
...
def main():
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if args.swa and epoch >= args.swa_start and (epoch - args.swa_start) % args.swa_c_epochs == 0:
            ...
            eval_swa_clean_acc1 = eval_clean(args, epoch, test_loader, swa_model)
            eval_swa_pgd_acc1 = eval_pgd(args, epoch, test_loader, swa_model)
            logging.info('SWA: Eval accuracy: \t %.4f, Eval robustness: \t %.4f', eval_swa_clean_acc1, eval_swa_pgd_acc1)

            writer.add_scalars('Eval/SWA_SA', {'swa': eval_swa_clean_acc1, 'standard': eval_clean_acc1}, epoch)
            writer.add_scalars('Eval/SWA_RA', {'swa': eval_swa_pgd_acc1, 'standard': eval_pgd_acc1}, epoch)

            if eval_swa_pgd_acc1 > best_swa_pgd_acc:
                best_swa_pgd_acc = eval_swa_pgd_acc1
                torch.save(swa_model.state_dict(),
                           os.path.join(model_dir, 'swa_best_model_' + str(args.trial) + '.pt'))

        elif args.swa: # store the accuracy of standard model to complete the curve.
            writer.add_scalars('Eval/SWA_SA', {'swa': eval_clean_acc1, 'standard': eval_clean_acc1}, epoch)
            writer.add_scalars('Eval/SWA_RA', {'swa': eval_pgd_acc1, 'standard': eval_pgd_acc1}, epoch)
