...
class ImagenetSubset(TensorDataset):
    def __init__(self, dataset):
        ...
        # tmp = random.sample(range(0,999), 10)
        # self.classes = np.array([1,2,3,4,5])
        self.classes = np.array([5, 127, 172, 269, 420, 532, 595, 864, 871, 988])
        self.indices = []
        self.new_targets = []
...
class Loaders:
    ...
    def __init__(
        ...
        self.data_dir = '/home/mnt/datasets/ImageNet2012'
        # self.data_dir = '/mnt/nfsdisk/data/imagenet'

    def trainset(self):
        trainpath = os.path.join(self.data_dir, 'train')
        trainset = ImageFolder(root=trainpath, transform=self.train_transforms)
        return ImagenetSubset(trainset)

    def testset(self):
        testpath = os.path.join(self.data_dir, 'val')
